...
type SearchQuery struct {
	Channel_name string `query:"channel_name" json:"channel_name,omitempty"`
}

// type virtual_warehouse_user_details struct {
...
// //register for virtual_warehouse
type (
	VirtualWarehouse_Request struct {
		VirtualWarehouse_id uint      `json:"virtual_warehouse_id"`
		Full_name           string    `json:"full_name"`
		Email               string    `json:"email"`
		Mobile_no           int64     `json:"mobile_no" `
		Date_of_birth       string    `json:"date_of_birth"`
		Bank_info           Bank_info `json:"bank_info"`
		KYC_info            Kyc_info  `json:"kyc_info"`
	...
	Bank_info struct {
		...
		Bank_statement   map[string]interface{} `json:"bank_statement"`
		Cancelled_cheque map[string]interface{} `json:"cancelled_cheque"`
	}

	Kyc_info struct {
		Iec_id         map[string]interface{} `json:"iec"`
		Passport_id    map[string]interface{} `json:"passport"`
		Adhaar_card_id map[string]interface{} `json:"adhaar_card"`
		Voter_id       map[string]interface{} `json:"voter_id"`
		Pan_card_id    map[string]interface{} `json:"pan_card"`
		Gstin_id       map[string]interface{} `json:"gstin"`
	}
)

// Create VirtualWarehouse response
type (
	VirtualWarehouseCreateResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseCreateResponse
)

// Update VirtualWarehouse response
type (
	VirtualWarehouseUpdateResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseUpdateResponse
)

// Get  VirtualWarehouse response
type (
	VirtualWarehouseGetResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseGetResponse
)

type (
	VirtualWarehouseGetAllResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data []VirtualWarehouse_Request
		}
	} //@ name VirtualWarehouseGetAllResponse
)

// Delete VirtualWarehouse response
type (
	VirtualWarehouseDeleteResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseDeleteResponse
)

// Delete VirtualWarehouseLines response
type (
	VirtualWarehouseLinesDeleteResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseLinesDeleteResponse
)

// ________________________________

type (
	Virtual_Warehouse_Details struct {
		VirtualWarehouse_id            uint                           `json:"virtual_warehouse_id"`
...
// Create VirtualWarehouse Detail response
type (
	VirtualWarehouseDetailCreateResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseDetailCreateResponse
)

// Update VirtualWarehouse Detail response
type (
	VirtualWarehouseDetailUpdateResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseDetailUpdateResponse
)

// Get  VirtualWarehouse Detail response
type (
	VirtualWarehouseDetailGetResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseDetailGetResponse
)

type (
	VirtualWarehouseDetailGetAllResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data []Virtual_Warehouse_Details
		}
	} //@ name VirtualWarehouseDetailGetAllResponse
)

// Delete VirtualWarehouse Detail response
type (
	VirtualWarehouseDetailDeleteResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data interface{}
		}
	} //@ name VirtualWarehouseDetailDeleteResponse
)

type (
	AvailableVirtualWarehousesResponse struct {
		Body struct {
			Meta response.MetaResponse
			Data []VirtualWarehouse
		}
	} //@ name AvailableVirtualWarehousesResponse
)
